...
exec(View, SubsetSize, ViewSize, ExpectedNodes, Id, Turn) -> % Should subset size be adapted, were part of nodes to die ?
	...
	receive
		{request, ReqNodes, ReqNode} ->  			
			...
			exec(NewView, SubsetSize, ViewSize, ExpectedNodes, Id, Turn);
		{response, RespNodes, RespNode} ->
			...
			exec(NewView, SubsetSize, ViewSize, NewExpected, Id, Turn);
		period ->
			NewView = inc(View),
			...
			case length(NewView)==0 of
				...
				false ->
					...
					timer:send_after(10000, period),
					timer:send_after(7000, {timeout, Oldest}),
					exec(reset_Q(NewView, Oldest, []), SubsetSize, ViewSize, [{Oldest,ReqNodes}] ++ ExpectedNodes, Id, Turn+1)
				end;				
		{timeout, ToCheck} ->
			case  lists:member(ToCheck, [Node || {Node, _} <- ExpectedNodes]) of 
				...
				false ->%[[],[{'25',0},{'12',0},{'40',0},{'7',0},{'39',0}],2,{'39',0}]
					exec(View, SubsetSize, ViewSize, ExpectedNodes, Id, Turn)
			end
	end.
...
start_nodes(Cur, Cluster, N, ViewSize, SubsetSize) ->
	...
	NodePid = spawn(?MODULE, exec, [View, SubsetSize, ViewSize, [], Node, 0]),
