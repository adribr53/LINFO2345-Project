...
init_from_list(Id, IdList) -> create_from_list(list_to_atom(integer_to_list(Id)), IdList, []).

create_from_list(_, [], CurList) 				-> CurList;
create_from_list(Id, CandidateList, CurList) 	->
	Candidate = list_to_atom(integer_to_list(lists:last(CandidateList))),
	case Id==Candidate of 
		true 	-> create_from_list(Id, lists:droplast(CandidateList), CurList);
		false 	-> create_from_list(Id, lists:droplast(CandidateList), [{Candidate,0} | CurList])
	end.
